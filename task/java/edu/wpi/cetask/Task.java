...
public class Task extends Instance {
   ...
   protected Task (TaskClass type, TaskEngine engine) {
      ...
      synchronized (bindings) {
         ...
         // call to updateBindings, but during constructor, steps are not yet
         // added
   ...
   public boolean equals (Object object) {
      ...
      synchronized (bindings) {
         try {
            ...
            return (Boolean) super.evalCondition(equals, compiledEquals,
                  "equals");
         } finally {
   ...
   static Compiled compiledEquals, compiledHashCode, compiledCloneThis,
         compiledCloneSlot;

   /**
    * Return value of the named slot.<br>
    * <br>
    * Note that this method returns a Java object, but the <em>real</em> values
    * are stored in JavaScript and must be converted to Java objects when
   ...
   public Object getSlotValue (String name) {
      ...
      if ( isScriptable(name) ) {
         ...
         return eval("$this." + name, "getSlotValue");
   }

   private void checkIsSlot (String name) {
      if ( !getType().isSlot(name) )
         throw new IllegalArgumentException(name + " is not slot of "
            + getType());
   }
   ...
   protected Boolean getSlotValueBoolean (String name) {
      ...
      if ( isScriptable(name) ) {
         ...
         return evalCondition("$this." + name, "getSlotValueBoolean");
   }
   ...
   private boolean isScriptable (String name) {
      ...
      return TaskEngine.SCRIPTABLE && slot.getType() != null
         && slot.getJava() == null;
   }
   ...
   public boolean isDefinedSlot (String name) {
      checkIsSlot(name);
      return TaskEngine.SCRIPTABLE ? engine.isDefined(engine.get(
            bindings.get("$this"), name))
   ...
   public void setSlotValue (String name, Object value, boolean check) {
      ...
      if ( !check || checkSlotValue(name, type, value) ) {
         ...
         failCheck(name, value.toString(), "setSlotValue");
   }
   ...
   protected void checkCircular (String name, Object value) {
      if ( value == this )
         ...
         throw new RuntimeException(
               "Attempting to store pointer to self in slot " + name + " of "
                  + this);
   }

   private boolean checkSlotValue (String name, String type, Object value) {
      if ( type == null || (type.equals("boolean") && value instanceof Boolean)
      ...
      if ( slot instanceof Input && ((Input) slot).isOptional()
         && value == null )
         return true;
      if ( slot.getJava() != null )
         return slot.getJava().isInstance(value);
      if ( engine.getScriptEngine() instanceof JintScriptEngine // for Unity
                                                                // only
         && Utils.startsWith(type, "Packages.") )
         try {
            return Class.forName(type.substring(9)).isInstance(value);
      ...
      synchronized (bindings) {
         try {
            ...
            return Utils.booleanValue(eval( // workaround bug:
                                            // "$$value instanceof Date" returns
                                            // false
                  type.equals("Date") ? "$$value.getUTCDate !== undefined"
                     : ("$$value instanceof " + type), "checkSlotValue"));
         } finally {
            bindings.remove("$$value");
         }
      }
   ...
   static String makeExpression (String self, TaskClass type, String name,
      ...
      buffer.append("if ( ")
            .append(checkExpression(type, name))
            .append(" ) {")
            ...
            .append("']")
            .append(
                  " = $$value; $$value = true; } else $$value = false; $$value");
      return buffer.toString();
   }

   void failCheck (String name, String expression, String where) {
      throw new IllegalArgumentException("Type error: Cannot assign result of "
         + expression + " to " + getType().getSlotType(name) + " slot " + name
         + " of " + getType() + " in " + where);
   }
   ...
   private static String checkExpression (TaskClass task, String name) {
      ...
      // allow any slot to be set to null (for "optional" inputs)
         ("$$value === null || " + ("boolean".equals(type)
            ? "typeof $$value == \"boolean\"" : "string".equals(type)
               ? "typeof $$value == \"string\"" : "number".equals(type)
                  ? "typeof $$value == \"number\"" :
   ...
   /**
    * Similar to {@link #setSlotValueScript(String,String,String)} but with
    * extra bindings to use for evaluation of expression (instead of task
    * bindings)
    */
   ...
   public Boolean isAchieved () {
      if ( achieved != null )
         return achieved; // overrides condition
      ...
      if ( !getType().hasModifiedInputs() || !occurred() )
         ...
         if ( clonedInputs == null ) // not checking each modified inputs
            throw new IllegalStateException(
                  "Modified inputs have not been cloned: " + this);
         synchronized (bindings) {
   ...
   @Override
   protected Boolean evalCondition (String condition, Compiled compiled,
         String where) {
   ...
   /**
    * Test whether all declared input slots (i.e., not including "external")
    * have defined values.
    * 
    * @see #getSlotValue(String)
    */
   ...
   public long getWhen () {
      try {
         ...
         engine.evalDouble("$this.when.getTime()", bindings, "getWhen")
               .longValue();
   ...
   public Boolean getSuccess () {
      return getSlotValueBoolean("success");
   }
      ...
      if ( condition != null ) {
         ...
         if ( slot == null ) {
            ...
            if ( Utils.isFalse(condition) ) {
               ...
               setSuccess(condition);
               getErr().println(
                     "WARNING: Ignoring true success slot in " + this);
            } else if ( sufficient )
               // if slot indicates failure, then be conservative
               getErr().println(
                     "WARNING: Ignoring true sufficient postcondition in "
                        + this);
         }
      }
      return getSuccess();
   ...
   public void setExternal (boolean external) {
      if ( !isPrimitive() )
         throw new UnsupportedOperationException(
               "Cannot set external slot of non-primitive task: " + this);
      setSlotValue("external", external);
   }

   /**
    * Test whether this task <em>must</em> be executed by user. It must be be
    * executed by the user iff the 'external' slot is true. Always returns false
    * for non-primitive tasks.
    * <p>
    * Note that both {@link #isUser()} and {@link #isSystem()} may return false,
    * but both may not return true.
    * 
    * @see #canUser()
    */
   public boolean isUser () {
      return Utils.isTrue(getExternal());
   }

   /**
    * Test whether this task <em>must</em> be executed by system. It must be
    * executed by system iff the 'external' slot is false. Always returns false
    * for non-primitive tasks.
    * <p>
    * Note that both {@link #isUser()} and {@link #isSystem()} may return false,
    * but both may not return true.
   ...
   public boolean isSystem () {
      return Utils.isFalse(getExternal());
   }

   /**
    * Test whether this task <em>can</em> be executed by user. It can be
    * executed by user iff it is primitive and it is not true that it must be
    * executed by system. Always returns false for non-primitive tasks.
    * <p>
    * Note that both {@link #canUser()} and {@link #canSystem()} may return
    * true, but both may not return false.
    * 
    * @see #isUser()
    */
   public boolean canUser () {
      return !isSystem() && isPrimitive();
   }

   /**
    * Test whether this task <em>can</em> be executed by system. It can be
    * executed by system iff it is primitive and it is not true that it must be
    * executed by user. Always returns false for non-primitive tasks.
    * <p>
    * Note that both {@link #canUser()} and {@link #canSystem()} may return
    * true, but both may not return false.
    * 
    * @see #isSystem()
    */
   public boolean canSystem () {
      return !isUser() && isPrimitive();
   }
    ...
    */
   public void setUnexplained (boolean unexplained) {
      this.unexplained = unexplained;
   }

   /**
    * Return modifiable list of decomposition classes <em>applicable</em> to
    * this task and not rejected.
    * 
   ...
   public List<DecompositionClass> getDecompositions () {
      ...
      for (Iterator<DecompositionClass> i = decomps.iterator(); i.hasNext();) {
         ...
         // remove decompositions not applicable to or rejected for this task
         // instance
    ...
    * 
    * @param contributes plan to which this occurrence has been found to
    *           contribute or null if no such plan could be found
    ...
    * instances match (symmetric relationship) iff they are both instances of
    * the same type and the values of each corresponding slot are either equal
    * or one is undefined.
    * 
   ...
   public boolean matches (Task goal) {
      if ( goal == this )
         return true;
      if ( !getType().equals(goal.getType()) )
         return false;
      for (String name : getType().inputNames)
         if ( !matchesSlot(goal, name) )
            return false;
   ...
   /**
    * Test whether this task either matches goal of given plan or could be
    * viewed as a part of the plan for given task, i.e., whether given plan
    * explains this task.
    * <p>
    * This method is for use in extensions. In CETask, it is equivalent to
    * {@link #matches(Task)}.
    */
   ...
   /**
    * Test whether this task can indirectly contribute to a task of given type.
    * Returns false if {@link #contributes(TaskClass)} returns true. (For
   ...
   /**
    * Copies the values of defined slots <em>from</em> given task to this task.
    * Note this side-effects this task, overwriting existing slot values.
    * 
   ...
   /**
    * If named slot of given task is defined, copy value <em>from</em> slot name
    * of given task to given slot of this task.
    * 
   ...
   boolean copySlotValue (Task from, String fromSlot, String thisSlot,
      ...
      if ( TaskEngine.SCRIPTABLE ) {
         ...
         if ( onlyDefined && !engine.isDefined(value) )
            return false;
         if ( check )
            checkSlotValue(thisSlot, getType().getSlotType(thisSlot), value);
         ...
         if ( onlyDefined && !from.isDefinedSlot(fromSlot) )
            return false;
   ...
   /**
    * Simulate execution of this task in each of the given hypothetical worlds.
    * <p>
    * For a primitive task, the resulting worlds include one world for each
    * input world in which this task is applicable and does not fail and in
    * which this task's grounding script has been evaluated to modify the world
    * (except for when this task's sufficient postcondition is true in the input
    * world, in which case the grounding script is not evaluated).
    * <p>
    * For a non-primitive task, the resulting worlds include all the worlds
    * resulting from applicable decomposition class choices that do not fail.
    * <p>
    * NB: World object class must provide a public copy constructor.
    * 
   ...
   protected void eval (Plan plan) {
      ...
      if ( type.getPostcondition() != null )
         for (Input input : type.declaredInputs) {
            ...
            if ( modified != null )
               engine.put(clonedInputs, modified.name,
                     getSlotValue(modified.name));
         }
   }
   ...
   private void modifiedOutputs () {
      ...
      for (Input input : type.declaredInputs) {
         ...
         if ( modified != null ) {
            ...
            if ( output == null )
               setSlotValue(modified.name, value);
            else if ( output != value )
               throw new IllegalStateException(
                     "Output of modified input not identical " + input.name);
         }
      }
   }
   ...
   /**
    * Clone and cache copy of named input <em>before</em> modification (for use
    * evaluating postconditions). Note that for reported actions by user, this
    * method should be called before done method.
    */
   public void cloneInput (String name) {
      Slot slot = getType().getSlot(name);
      if ( slot == null || !(slot instanceof Input)
         || ((Input) slot).getModified() == null )
   ...
   /**
    * Return appropriate and applicable script to ground this primitive task, or
    * null if none.
    */
   public Grounding getGrounding () {
      Grounding script = getType().getGrounding();
      return script == null ? null : Utils.isFalse(script.isApplicable(this))
         ? null : script;
   }

   public String getProperty (String key) {
      return getType().getProperty(key);
   }
   ...
   protected String toPropertyString (Object value) {
      ...
      boolean debug = TaskEngine.DEBUG, print = TaskEngine.PRINT_TASK, verbose = TaskEngine.VERBOSE;
      try {
    ...
    */
   public List<DecompositionClass> getRejected () {
      return rejected;
   }
   ...
   public void reject (DecompositionClass decomp) {
      if ( rejected.isEmpty() )
         rejected = new ArrayList<DecompositionClass>();
      rejected.add(decomp);
   }
   ...
   public String toString () {
      ...
      if ( isPrimitive() ) {
         ...
         if ( external != null )
            buffer.insert(0,
                  external ? engine.getExternalName() : engine.getSystemName());
      }
      String id = getType().getPropertyId();
      return buffer.length() == 0 ? id : buffer.insert(0, '(').insert(0, id)
            .append(')').toString();
   }
   ...
   public List<Object> getDeclaredSlotValues () {
      ...
      for (Output output : type.declaredOutputs)
         ...
         list.add(type.getModifiedInput(output) == null
            ? getSlotValueIf(output.name) : modifiedOutput);
      return list;
   }
   ...
   protected StringBuilder argListBuilder (List<Object> args) {
      for (int i = args.size(); i-- > 0;) {
         ...
         if ( arg == undefined || arg == modifiedOutput )
            args.remove(i);
         else
            break;
      }
   ...
   private String toString (Object value) {
      return value instanceof String ? ("\"" + value + "\"") : engine
            .toString(value);
   }

   @Override
   protected String toStringVerbose () {
      StringBuilder buffer = new StringBuilder(super.toStringVerbose())
            .append("={ ");
      boolean first = true;
      for (String name : getType().inputNames)
      ...
      if ( TaskEngine.DEBUG && isDefinedSlot("when") ) {
         ...
         long when = getWhen();
         first = appendSlot(buffer, "when", start == 0 ? DateFormat
               .getDateTimeInstance(DateFormat.SHORT, DateFormat.MEDIUM)
               .format(new Date(when)) : Long.toString((when - start) / 100), // tenth
                                                                              // second
                                                                              // accuracy
                                                                              // enough
               first);
   ...
   private boolean appendSlot (StringBuilder buffer, String name, boolean first) {
      return isDefinedSlot(name) ? appendSlot(buffer, name,
            toString(getSlotValue(name)), first) : first;
   }
   ...
   public final String format () {
      return engine.format(this);
   }

   /**
    * Return compact human-readable string for this task. For overriding by
    * extensions of Task.
    */
   ...
   protected String formatSlot (String name, boolean defined) {
      if ( !defined )
         return getType().formatSlot(name, false);
      Object value = getSlotValue(name);
      return value instanceof Task ? ((Task) value).formatTask()
         : clonedInputs == null ? getSlotValueToString(name) : engine
               .toString(getClonedInputsSlotValue(name));
   }
   ...
   private StringBuilder formatSlots (List<String> slots, int size, String key,
         String and) {
      StringBuilder buffer = new StringBuilder();
      ...
      if ( buffer.length() > 0 )
         buffer.insert(0, ' ').insert(0, engine.getProperty(key))
               .insert(0, ' ');
      return buffer;
   }
