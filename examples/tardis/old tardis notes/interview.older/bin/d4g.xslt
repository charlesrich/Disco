...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="t:task[not(starts-with(@id, '_'))]" mode="rem-spur">
    <xsl:copy-of select="."/>
  </xsl:template>
  ...
  <xsl:template match="t:task[@is-spurious='true'][starts-with(@id, '_')]" mode="rem-spur">
    <xsl:variable name="rev-ref-task" select="key('rev-access-task', @id)"/>
   
    <xsl:if test="not(starts-with($rev-ref-task/@id, '_'))">
      <xsl:copy>
  ...
  <xsl:template match="t:task[not(@is-spurious='true')][starts-with(@id, '_')]" mode="rem-spur"> 
    <xsl:copy>
      <xsl:attribute name="id">
        <xsl:value-of select="@id"/>
      </xsl:attribute>
